...
```xml
 <!-- 这个插件，可以将应用打包成一个可执行的jar包；无需安装tomcat服务器-->
 <!-- 在lifecycle中点击 package 即可以打包-->
    <build>
        <plugins>
            <plugin>
                <groupId>org.springframework.boot</groupId>
...
将这个应用打成jar包，直接使用`java -jar xxxx.jar `的命令进行执行；
...
```java
@AutoConfigurationPackage // 主配置类所在包及其子包下的组件导入spring容器
...
public class Person {
   ...
   //lastName必须是邮箱格式
    @Email
    //@Value("${person.last-name}") spring的底层注解
    private String lastName;
    //@Value("#{11*2}")
    private Integer age;
    //@Value("true")
