...
2. Activiti的架构、类关系图
   ...
   ~~~
   获取流程引擎的工具类
   ProcessEngines 使用默认方式获取配置文件，构造流程引擎。配置文件名字：activiti.cfg.xml，放在classpath下
   ProcessEngineConfiguration 可以自定义配置文件名
   使用上面两个工具类，都可以获得流程引擎：ProcessEngine
   ProcessEngine:流程引擎，获取各种服务的接口。
   服务接口：用于流程的部署/执行/管理。使用这些接口就是在操作数据表
   	- RepositoryService 资源管理类
   	- RuntimeService    运行时管理类
   	- TaskService       任务管理类
   	- HistoryService    历史数据管理类
   	- ManagementService 流程引擎管理列
   ~~~
...
4. 流程符号、画图流程
   ...
   使用流程设计器画出流程图
   创建bpmn文件，bpmn本质上是xml文件，因为安装了工具，才能可视化看到流程内容
   - 将bpmn文件改为xml文件，在这个文件上右键Diagrams->show BPMN2.0 Desinger->导出
   ~~~
...
10. 流程删除
    ...
    ~~~
    根据流程部署id进行删除，如果有两个参数，则会对未完成的任务进行级联删除，第二个参数为true
    ~~~

11. 流程资源下载

    ~~~
    如果用户想要查看流程的资源文件，可以从数据库中把资源文件下载到本地
    ~~~
