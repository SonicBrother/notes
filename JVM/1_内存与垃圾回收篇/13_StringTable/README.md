...
/**
 * new String("a") + new String("b") 会创建几个对象？ 
 * StringBuilder/String(a)/常量池a/String(b)/常量池b/ 深入剖析：new String(ab)-->常量池不存在ab
 * @author: 陌溪
 * @create: 2020-07-11-11:17
 */
public class StringNewTest {
...
```java
String s = new String("1");  // 在常量池中已经有了 创建两个对象：常量池1和堆中1
s.intern(); // 将该对象放入到常量池。但是调用此方法没有太多的区别，因为已经存在了1
String s2 = "1";
System.out.println(s == s2); // jdk6:false jdk7/8:false

String s3 = new String("1") + new String("1"); // 只有堆空间中有11/执行完以后常量池中没有11
s3.intern();// 在字符串常量池中生成 11
// 如何理解：jdk6:创建了一个新的对象11，也就是有了新的地址
			jdk7/8:此时常量池中并没有创建11，而是创建一个指向堆空间中new String("11")的地址
String s4 = "11"; // jdk7/8使用上一行代码指向的 堆空间中的11的地址
